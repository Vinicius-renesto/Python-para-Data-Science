...
def mostra_grafico_bar():
    ...
    if 'Valor' in dados.columns and 'Produtos' in  dados.columns:
        ...
        valor_vendas = dados.groupby('Região')['Valor'].sum()
...
def mostra_grafico_plot():
    ...
    if 'Valor' in dados.columns and 'Produtos' in  dados.columns:
        ...
        valor_regiao.plot(kind ='plot', ax=ax, color = 'green')
        ax.set_title('Valor')
        ax.set_ylabel('Valores')
        ax.tick_params(axis = 'x', rotation = 45)
    else:
...
ttk.Button(btn_frame, text='Gerar o grafico barras', command = mostra_grafico_bar).pack(side='left', padx=5)
ttk.Button(btn_frame, text='Gerar o grafico plot', command = mostra_grafico_plot).pack(side='left', padx=5)
